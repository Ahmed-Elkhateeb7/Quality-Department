...
export const Products: React.FC<ProductsProps> = ({ products, setProducts, requestAuth, role }) => {
  ...
  const resetForm = () => {
    setFormData({ name: '', specs: '', defects: '', status: 'pending', image: '', manufacturer: '' });
                  ...
                  <div className="space-y-6">
                     <div>
                        <div className="flex flex-wrap gap-2 mb-3">
                            <span className={`px-4 py-1.5 rounded-full text-sm font-black border shadow-sm ${getStatusColor(viewingProduct.status)}`}>
                                الحالة: {getStatusText(viewingProduct.status)}
                            </span>
